window.siteConfig = {
  business: {
    name: "Kelis Malone Photography",
    tagline: "Photography with warmth, style, and story",
    headline: "Timeless photography for modern couples, families, and brands.",
    subtext:
      "I create natural, editorial images that feel effortless while capturing real emotion.",
    email: "hello@example.com",
    phone: "(555) 123-4567",
    location: "Chicago, Illinois",
    bookingUrl: "https://calendly.com/",
  },
  hero: {
    primaryCtaText: "Book a Session",
    secondaryCtaText: "See Portfolio",
  },
  about: {
    headline: "Hi, I'm Kelis",
    text:
      "I help clients feel comfortable on camera so your photos look like your best, most natural self. From weddings and portraits to campaigns and events, my process is collaborative and easy from start to finish.",
    image:
      "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?auto=format&fit=crop&w=1200&q=80",
  },
  services: [
    {
      title: "Weddings",
      description: "Full-day coverage, timeline planning support, and online gallery delivery.",
    },
    {
      title: "Portrait Sessions",
      description: "Individual, couples, maternity, and family sessions on location or in studio.",
    },
    {
      title: "Brand & Editorial",
      description: "Visual storytelling for entrepreneurs, teams, and creative campaigns.",
    },
    {
      title: "Events",
      description: "Corporate and private event coverage with quick turnaround options.",
    },
  ],
  portfolio: {
    intro: "Replace these photos with your own best work to match your style and niche.",
    items: [
      {
        image:
          "https://images.unsplash.com/photo-1519741497674-611481863552?auto=format&fit=crop&w=1200&q=80",
        alt: "Wedding couple in evening light",
        caption: "Downtown rooftop wedding",
      },
      {
        image:
          "https://images.unsplash.com/photo-1522673607200-164d1b6ce486?auto=format&fit=crop&w=1200&q=80",
        alt: "Portrait of a woman near window light",
        caption: "Editorial portrait session",
      },
      {
        image:
          "https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?auto=format&fit=crop&w=1200&q=80",
        alt: "Beauty branding detail shot",
        caption: "Brand detail series",
      },
      {
        image:
          "https://images.unsplash.com/photo-1511285560929-80b456fea0bc?auto=format&fit=crop&w=1200&q=80",
        alt: "Group at event smiling",
        caption: "Private event coverage",
      },
      {
        image:
          "https://images.unsplash.com/photo-1472746729193-36ad213ac4a5?auto=format&fit=crop&w=1200&q=80",
        alt: "Family portrait outdoors",
        caption: "Family sunset session",
      },
      {
        image:
          "https://images.unsplash.com/photo-1517841905240-472988babdf9?auto=format&fit=crop&w=1200&q=80",
        alt: "Entrepreneur personal branding portrait",
        caption: "Branding portrait",
      },
    ],
  },
  pricing: [
    {
      title: "Mini Session",
      price: "$350",
      details: "30 minutes, 15 edited images, online gallery",
    },
    {
      title: "Signature Session",
      price: "$650",
      details: "90 minutes, 45 edited images, 2 outfit changes",
    },
    {
      title: "Wedding Collections",
      price: "Starting at $2,800",
      details: "6+ hour coverage, second shooter options, timeline support",
    },
  ],
  testimonials: [
    {
      quote:
        "Kelis made us feel comfortable immediately. Our photos feel authentic and cinematic.",
      author: "Marina & Alex",
    },
    {
      quote:
        "Our brand photos finally look cohesive and premium. The session was organized and fun.",
      author: "The Gray Studio",
    },
    {
      quote:
        "The gallery was stunning and delivered quickly. We will absolutely book again.",
      author: "Nia R.",
    },
  ],
  social: [
    { label: "Instagram", url: "https://instagram.com/" },
    { label: "Pinterest", url: "https://pinterest.com/" },
    { label: "TikTok", url: "https://tiktok.com/" },
  ],
  theme: {
    bg: "#f6f1e9",
    surface: "#ffffff",
    text: "#25211d",
    muted: "#665f57",
    primary: "#9a3412",
    primaryContrast: "#fff7ed",
    accent: "#d97706",
    border: "#e4dbd1",
    heroImage:
      'linear-gradient(120deg, rgba(27, 20, 14, 0.64), rgba(67, 40, 24, 0.38)), url("https://images.unsplash.com/photo-1516035069371-29a1b244cc32?auto=format&fit=crop&w=1600&q=80")',
  },
  footerNote: "All images and content copyright Kelis Malone Photography.",
};
